    ...
    class NetworkHostTreeNode : TreeNode, IBeforeExpand{
        ...
        private string GetOsImageKey() {
            if(networkHost.OS==PacketParser.NetworkHost.OperatingSystemID.Windows)
                ...
                return "linux";
            else if(networkHost.OS==PacketParser.NetworkHost.OperatingSystemID.MacOS)
                return "macos";
        ...
        public void BeforeExpand() {
            ...
            if (networkHost.MacAddress != null) {
                TreeNode nicNode = this.Nodes.Add("nic", "MAC: " + networkHost.MacAddress.ToString() + " (" + PacketParser.Fingerprints.MacCollection.GetMacCollection(System.IO.Path.GetFullPath(System.Windows.Forms.Application.ExecutablePath)).GetMacVendor(networkHost.MacAddress) + ")", "nic", "nic");
            ...
            foreach(string osCounterName in networkHost.OsCounterNames) {
                osNode.Nodes.Add(osCounterName, osCounterName+": "+networkHost.GetOsDetails(osCounterName));
            }
            ...
            if (this.hostDetailsGenerator != null && !networkHost.ExtraDetailsList.ContainsKey(this.hostDetailsGenerator.GetDefaultKeyName())) {
                ...
                for (int i = 0; i < extraDetails.Count; i++) {
                    ...
                    //networkHost.HostDetailCollection.Add(extraDetails.Keys[i], extraDetails[i]);
                }
            }
