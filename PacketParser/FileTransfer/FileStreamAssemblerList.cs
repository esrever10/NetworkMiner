...
namespace PacketParser.FileTransfer {
    public class FileStreamAssemblerList : PopularityList<string, FileStreamAssembler> {
        ...
        /*internal bool ContainsActiveAssembler(NetworkHost sourceHost, ushort sourcePort, NetworkHost destinationHost, ushort destinationPort, bool tcpTransfer) {
            string id=GetAssemblerId(sourceHost, sourcePort, destinationHost, destinationPort, tcpTransfer);
            return ContainsAssembler(id, true);
        }*/
        ...
        /*
        //this one is slower! It goes in O(poolSize) instead of constantTime
        internal IEnumerable<FileStreamAssembler> GetAssemblers(PacketHandlers.FtpPacketHandler.FtpSession ftpSession) {
            return GetAssembler(ftpSession
        }
         * */
