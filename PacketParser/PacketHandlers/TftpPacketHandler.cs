    ...
    class TftpPacketHandler : AbstractPacketHandler, IPacketHandler {
        ...
        private bool TryCreateNewAssembler(out FileTransfer.FileStreamAssembler assembler, FileTransfer.FileStreamAssemblerList fileStreamAssemblerList, Packets.TftpPacket tftpPacket, NetworkHost sourceHost, ushort sourcePort, NetworkHost destinationHost) {//destinationPort is not needed
            ...
            if(tftpPacket.OpCode==Packets.TftpPacket.OpCodes.ReadRequest) {
                try {
                    assembler=new FileTransfer.FileStreamAssembler(fileStreamAssemblerList, destinationHost, Packets.TftpPacket.DefaultUdpPortNumber, sourceHost, sourcePort, false, FileTransfer.FileStreamTypes.TFTP, tftpPacket.Filename, "", tftpPacket.OpCode.ToString()+" "+tftpPacket.Mode.ToString()+" "+tftpPacket.Filename, tftpPacket.ParentFrame.FrameNumber, tftpPacket.ParentFrame.Timestamp);
                ...
                try {
                    assembler=new FileTransfer.FileStreamAssembler(fileStreamAssemblerList, sourceHost, sourcePort, destinationHost, Packets.TftpPacket.DefaultUdpPortNumber, false, FileTransfer.FileStreamTypes.TFTP, tftpPacket.Filename, "", tftpPacket.OpCode.ToString()+" "+tftpPacket.Mode.ToString()+" "+tftpPacket.Filename, tftpPacket.ParentFrame.FrameNumber, tftpPacket.ParentFrame.Timestamp);
                    fileStreamAssemblerList.Add(assembler);
                }
                catch(Exception e) {
                    //throw new Exception("Error creating assembler for TFTP file transfer", e);
            ...
            if(tftpPacket.OpCode==Packets.TftpPacket.OpCodes.Data) {
                if(!assembler.IsActive) {
                    ...
                    if(assembler.SourcePort!=sourcePort || assembler.DestinationPort!=destinationPort) {
                        ...
                        assembler=new FileTransfer.FileStreamAssembler(fileStreamAssemblerList, sourceHost, sourcePort, destinationHost, destinationPort, false, FileTransfer.FileStreamTypes.TFTP, assembler.Filename, assembler.FileLocation, assembler.Details, tftpPacket.ParentFrame.FrameNumber, tftpPacket.ParentFrame.Timestamp);
                        fileStreamAssemblerList.Add(assembler);
                    }
