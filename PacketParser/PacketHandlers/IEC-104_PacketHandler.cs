        ...
        public int ExtractData(NetworkTcpSession tcpSession, NetworkHost sourceHost, NetworkHost destinationHost, IEnumerable<Packets.AbstractPacket> packetList) {
            ...
            foreach (Packets.AbstractPacket p in packetList) {
                if (p.GetType() == typeof(Packets.IEC_60870_5_104Packet))
                    returnValue = ExtractData(tcpSession, sourceHost, destinationHost, (Packets.IEC_60870_5_104Packet)p);
            }

            return returnValue;
        }

        private int ExtractData(NetworkTcpSession tcpSession, NetworkHost sourceHost, NetworkHost destinationHost, Packets.IEC_60870_5_104Packet iec104Packet) {
            //TODO extract data
            if (iec104Packet.AsduData != null && iec104Packet.AsduData.Length > 0) {
                ...
                if(parameters.Count > 0)
                    base.MainPacketHandler.OnParametersDetected(new Events.ParametersEventArgs(iec104Packet.ParentFrame.FrameNumber, sourceHost, destinationHost, sourcePort.ToString(), destinationPort.ToString(), parameters, iec104Packet.ParentFrame.Timestamp, details));
            }
