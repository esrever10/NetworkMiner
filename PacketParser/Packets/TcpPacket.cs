...
namespace PacketParser.Packets {
    ...
    public class TcpPacket : AbstractPacket, ITransportLayerPacket {
        ...
        public class Flags{
            ...
            public byte RawData { get { return this.flagData; } }
        ...
        public override IEnumerable<AbstractPacket> GetSubPackets(bool includeSelfReference) {
            ...
            if(PacketStartIndex+dataOffsetByteCount<PacketEndIndex) {
                ...
                /*
                if(sourcePort==21) {//FTP server->client
                    try {
                        packet=new FtpPacket(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex, false);
                    }
                    catch(Exception) {
                        packet=new RawPacket(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex);
                    }
                }
                else if(destinationPort==21) {//FTP client->server
                    try {
                        packet=new FtpPacket(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex, true);
                    }
                    catch(Exception) {
                        packet=new RawPacket(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex);
                    }
                ...
                else if(sourcePort==80 || destinationPort==80 || sourcePort==8080 || destinationPort==8080) {
                    if(!HttpPacket.TryParse(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex, out packet)) {
                        packet=new RawPacket(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex);
                    }
                }
                    ...
                    sourcePort==443 || destinationPort==443 ||
                    sourcePort==465 || destinationPort==465 ||
                    sourcePort==563 || destinationPort==563 ||
                    sourcePort==992 || destinationPort==992 ||
                    sourcePort==993 || destinationPort==993 ||
                    sourcePort==994 || destinationPort==994 ||
                    sourcePort==995 || destinationPort==995 ||
                    sourcePort==989 || destinationPort==989 ||
                    sourcePort==990 || destinationPort==990) {

                    try {
                        packet=new SslPacket(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex);
                    }
                    catch {
                        packet=new RawPacket(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex);
                    }
                ...
                else if(sourcePort==1433 || destinationPort==1433) {
                    try {
                        packet=new TabularDataStreamPacket(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex);
                    }
                    catch {
                ...
                else if(destinationPort==4070) {//client->server
                    if(!SpotifyKeyExchangePacket.TryParse(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex, true, out packet)) {
                        packet=new RawPacket(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex);
                    }

                }
                else if(sourcePort==4070) {//server->client
                    if(!SpotifyKeyExchangePacket.TryParse(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex, false, out packet)) {
                        packet=new RawPacket(ParentFrame, PacketStartIndex+dataOffsetByteCount, PacketEndIndex);
                    }
                }
                else* */
